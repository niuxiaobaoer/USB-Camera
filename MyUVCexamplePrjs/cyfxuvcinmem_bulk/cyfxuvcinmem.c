        ...
        uint32_t input)
{
    ...
    for (;;)
    {
        ...
        while (glIsApplnActive)
        {
            ...
            //status = CyU3PDmaChannelGetBuffer (&glChHandleUVCStream, &dmaBuffer,  CYU3P_WAIT_FOREVER);//deleted by qbc
            if (status != CY_U3P_SUCCESS)
            {
            	break;
            }
            ...
            headerOffset = 0;
            /* Check if this is the start of the frame.
             * Add header.
             * */

            if (frameOffset == 0)
            {
                headerOffset = CY_FX_UVC_MAX_HEADER;
                    ...
                    glVidFrameLen[frameIndex])
            {
                ...
                //CyU3PMemCopy ((dmaBuffer.buffer + headerOffset),
                        //(uint8_t *)&glUVCVidFrames[frameStart + frameOffset],
                       // (CY_FX_UVC_STREAM_BUF_SIZE - headerOffset));				//deleted by qbc

                frameOffset += (CY_FX_UVC_STREAM_BUF_SIZE - headerOffset);
            }
            else
            {
                /* SLP/ZLP*/
                ...
                if (commitLength)
                {
                   // CyU3PMemCopy ((dmaBuffer.buffer + CY_FX_UVC_MAX_HEADER),
                    //        (uint8_t *)&glUVCVidFrames[frameStart + frameOffset],
                    //        (glVidFrameLen[frameIndex] - frameOffset));							//deleted by qbc

                    /* Commit buffer length */
                    commitLength += CY_FX_UVC_MAX_HEADER;
            ...
            //status = CyU3PDmaChannelCommitBuffer (&glChHandleUVCStream, commitLength, 0);			//deleted by qbc
            if (status != CY_U3P_SUCCESS)
            {
                break;
            }
